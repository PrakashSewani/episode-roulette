...
- Current state: **v1.6.0 is published and pending Chrome Web Store review.** Phase 13 (Failure Reporting) is complete: every user-facing error snackbar carries a **Report** link into a `/report` form whose options are pre-selected from the failure context. The website (`episode-roulette-website` commit `5d221e9`) is deployed and live; the extension is `eaebb55`, tagged `v1.6.0`. No phase is `in progress`. **Safari is explicitly skipped by user decision, reaffirmed 2026-09-12:** the Apple Developer Program cost is not justified by demand.
- Item currently in progress: none. Phase 13 is closed; only manual follow-ups remain (listed under blockers and next action).
...
- Release evidence (recorded): tag `v1.6.0` pushed at `eaebb55`; tag-triggered `Release` run `35519926017` completed **success** in 23 s. The publish step logged `Created episode-roulette-v1.6.0.zip`, `Obtained access token`, upload `uploadState: 'SUCCESS'`, then publish `status: ['OK']` / `statusDetail: ['OK.']`, ending with `Package published to Chrome Web Store`. The refreshed `CHROME_REFRESH_TOKEN` was accepted, so the token exchange is healthy as of 2026-09-20.
- Blockers or unanswered questions: the `CHROME_REFRESH_TOKEN` was renewed on 2026-09-20 and is valid again, but the OAuth consent screen is still in **Testing**, so it will expire roughly seven days from issue (~2026-09-27). Either publish the consent screen to **Production** or migrate to the API v2 service-account flow before the next release, rather than refreshing a one-week token again. Outstanding manual checks: the live click-through of the report link from a **real provider failure** cannot be forced on demand (the destination page, the payload, and the submission path are verified; only the in-page click that opens it from a genuine failure is unproven), and the live uninstall redirect still needs a real uninstall. The Chrome Web Store dashboard description still needs the manual paste from `store-assets/store-listing.md`, including the new report-link wording. **The Chrome Web Store API v1.1 is supported only until 15 October 2026** — migrate `scripts/publish-chrome.mjs` to the v2 API before then. `npm run assert:safari` was not run locally because `xcodebuild` is unavailable on this machine.
...
- Exact next action: confirm v1.6.0 goes live in the dashboard after review, and paste the updated store description, permissions justification, remote-code text, and promo tiles there — the publish API cannot update listing copy. Before the next release, either move the OAuth consent screen to **Production** or migrate `scripts/publish-chrome.mjs` to the Chrome Web Store API v2 (hard deadline 15 October 2026), so the refresh token stops expiring weekly. When a real roll fails in the wild, the resulting report lands in KV at `submission:<ts>:<uuid>`; triage it with the User Report Triage table in `knowledge-transfer/maintenance-playbook.md`.
...
| 13. Failure Reporting | complete | Shipped as v1.6.0 (`eaebb55`, tag pushed 2026-09-20); website `/report` live (`5d221e9`); publish confirmed `uploadState: 'SUCCESS'`, `status: ['OK']`. Remaining manual check: snackbar Report click from a real provider failure. |
...
- [x] Bump `package.json` to `1.6.0` (done), push both repositories (website pushed as `5d221e9`), and tag `v1.6.0` after the refresh token is renewed.
- [x] Confirm the tag-triggered `Release` run succeeds and the store accepts the upload.
