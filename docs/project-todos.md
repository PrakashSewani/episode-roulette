...
- Exact next action: upload the regenerated promo tiles (`store-assets/promo-small-440x280.png`, `promo-large-920x680.png`, `marquee-1400x560.png`) in the dashboard — the store's listing assets are dashboard-only and cannot be changed by the publish API. While v1.5.0 is in review, also paste the updated store description, permissions justification, and remote-code text — the live copy still claims there is no background service worker, which no longer matches the package. Then confirm v1.5.0 goes live after approval. Before 15 October 2026, migrate the publish script to the Chrome Web Store API v2. For the next release, follow `docs/release.md#release-ordering`: deploy the website first, then bump `package.json`, run the gate, and push the tag. The `v1.5.0` tag points at `22b0a70`; the tooling fix is the later `4130f25` on `main`.
